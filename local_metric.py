...
if __name__ == "__main__":
    ...
    # 1) Leer directamente con pandas:
    df_val = pd.read_parquet("submission.parquet")  
    # Ahora df_val es un DataFrame con columnas ["sequence_id", "gesture"]

    # 2) Si quisieras leerlo primero con Polars y luego pasar a pandas, sería:
    # import polars as pl
    # df_pl = pl.read_parquet("val.parquet")
    # df_val = df_pl.to_pandas()

    # 3) Verifica que las columnas se llamen exactamente "sequence_id" y "gesture":
    print(df_val.columns)  # debe mostrar Index(['sequence_id','gesture'], ...)

    # 4) Carga también tu conjunto “ground truth” de validación, por ejemplo:
    df_true = pd.read_csv("val_split.csv")[["sequence_id","gesture"]]
    # agrupar por sequence_id y tomar la primera fila de cada grupo
    df_true = df_true.groupby("sequence_id").first().reset_index()

    print(df_true.shape)

    # 5) Llama al score:
    puntaje = score(df_true, df_val, row_id_column_name="sequence_id")
    print("Hierarchical F1 en validación:", puntaje)
